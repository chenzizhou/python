...
class Register(FlaskForm):
    ...
    password = PasswordField(label='密　码', validators=[InputRequired("密码不能为空"), check_login],render_kw={'placeholder': '输入密码'})
    email = StringField(label='邮 箱', validators=[InputRequired("邮箱不能为空"), Email('邮箱格式错误')],render_kw={'placeholder': '输入邮箱'})
...
def register_page():
    ...
    if form.validate_on_submit():
        ...
        print('添加用户')
        user1 = UserModel(username=form.username.data, password=generate_password_hash(form.password.data), email=form.email.data)
        # 增加用户到数据库
