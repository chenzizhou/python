...
class User:
    ...
    def make_scheme(self, driver, fa_name, fw_name):
        ...
        ##    ##提示框处理 确定 body > div.v-dialog.v-s-normal.v-t-confirm > div.v-dialog-footer.v-s-center > div.v-dialog-footer-btn.v-dialog-footer-btn-ok
        ##    driver.find_element_by_css_selector('body > div.v-dialog.v-s-normal.v-t-confirm > div.v-dialog-footer.v-s-center > div.v-dialog-footer-btn.v-dialog-footer-btn-ok').click()

        ##提示框处理 取消 body > div.v-dialog.v-s-normal.v-t-confirm > div.v-dialog-footer.v-s-center > div.v-dialog-footer-btn.v-dialog-footer-btn-ok
        driver.find_element_by_css_selector(
    ...
    def make_ywpz(self, driver):
        ...
        for element in elements:
            if element.text=='业务配置':
                ...
                break
        sleep(2)
        # cks=driver.find_elements_by_tag_name('span')
        # spanfor ck in cks:
        #     if ck.text=='查看':
        #         ck.parentNode().click()
        #         driver.find_element_class_name('index__addStyle___3HMma')
        #         driver.find_element_by_id('name').send_keys('巡检')
        #         driver.find_element_by_id('code').send_keys('巡检')
        driver.find_element_by_xpath('//*[@id="main"]/div[5]/div[2]/div[2]/div/div[2]'
                                      '/div[3]/div/div/div/div/div/div/div/div/table/tbody/tr[1]/td[3]/div/button[1]').click()

        print(123)
        driver.find_element_by_xpath('//*[@id="main"]/div[5]/div[2]/div[2]/div/div[2]/div[4]/div/div/div[2]').click()
        sleep(4)
        driver.find_element_by_id('name').send_keys('巡检')
        sleep(1)
        driver.find_element_by_id('code').send_keys('巡检')
        sleep(1)
        # 添加作业对象
        driver.find_element_by_xpath('//*[@id="main"]/div[5]/div[2]/div[2]'
                                     '/div/div[2]/div[5]/div/div/div/div/div[3]/div[1]/div[2]/div[1]/button[1]').click()
        sleep(1)
        driver.find_element_by_xpath('/html/body/div[17]/div/div/div/div[2]/div[2]/div/div/form/'
                                     'div[1]/div/div/div[2]/div/span/span/span/span[1]').click()
        sleep(8)
        driver.find_element_by_xpath('//span[@title="泽川供水"]').nextSibling().find_element_by_xpath('//span[@title="管段"]').click()
        print(123)
        sleep(10)